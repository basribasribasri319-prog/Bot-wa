...
//theme sticker reply
        ...
        if (m.message) {
            ...
            console.log(chalk.bgHex("#000000").bold(`▢ Ada pesan baru nih [ Shadow ]`));
            console.log(
...
//
   ...
   const fitur1 = () =>{
            var mytext = fs.readFileSync("./Budetzz.js").toString()
...
case "listcase": {
if (!isOwner) return reply(mess.owner)
const code = fs.readFileSync('Budetzz.js', 'utf8');
...
const namaFile = path.join(__dirname, 'Budetzz.js');
      const caseBaru = `${text}\n\n`;
...
case 'delcase': {
    ...
    break
      ...
      const getCase = async (caseName) => {
        try {
          const fileContent = await fs.promises.readFile('./Budetzz.js', "utf-8")
